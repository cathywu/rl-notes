...
class DQN(nn.Module, QFunction):
    ...
    def select_action(self, state, actions):
        ...
        if sample > eps_threshold:
            with torch.no_grad():
                ...
                return self.get_max_q(state, actions)
    ...
    def get_q_value(self, state, action):
        ...
        state_tensor = torch.tensor(state, device=device, dtype=torch.int64)
        q_value = self(state_tensor).gather(1, action)[0]
        return q_value
    ...
    def get_max_q(self, state, actions):
        with torch.no_grad():
            ...
            #print("\n\nstate tensor = " + str(state))
            return self(state_tensor).max(1).indices.view(1, 1).item()
    ...
    def update(self, transitions):
        ...
        non_final_next_states = torch.tensor([s for s in batch.next_state
                                                    if s is not None], dtype=torch.float32, device=device)
...
episode_rewards = learner.execute(episodes)
